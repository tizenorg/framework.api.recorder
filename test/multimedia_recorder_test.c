...
static gboolean mode_change()
{
    ...
    if (state != RECORDER_STATE_NONE)
    {
            ...
            || (state == RECORDER_STATE_PAUSED))
        {
            ...
            if (err < 0)
            {
                LOGE("exit recorder_cancel  = %x", err);
                return FALSE;
            }
        }
    ...
    init_handle();
    camera_create(CAMERA_DEVICE_CAMERA0, &hcamcorder->camera);
    LOGD("camera_create");
    camera_set_display(hcamcorder->camera,CAMERA_DISPLAY_TYPE_OVERLAY,GET_DISPLAY(display));
    ...
    while (!check) {
        ...
        case '1':
            ...
            recorder_create_videorecorder(hcamcorder->camera,&hcamcorder->recorder);
            check = TRUE;
            break;
        case '2':
            hcamcorder->mode= MODE_AUDIO;
            recorder_create_audiorecorder(&hcamcorder->recorder);
            recorder_attr_set_audio_device(hcamcorder->recorder,RECORDER_AUDIO_DEVICE_MIC);
            check = TRUE;
            break;
        case '3':
            hcamcorder->mode= MODE_AUDIO;
