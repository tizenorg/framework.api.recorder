...
static int __mm_recorder_msg_cb(int message, void *param, void *user_data){
	...
	switch(message){
		case MM_MESSAGE_READY_TO_RESUME:
			LOGW("Invalid _RECORDER_EVENT_TYPE_INTERRUPT_COMPLETED callback %p", handle->user_cb[_RECORDER_EVENT_TYPE_INTERRUPT_COMPLETED]);
			break;
...
int recorder_create_videorecorder(camera_h camera, recorder_h* recorder)
{
	...
	ret = mm_camcorder_get_attributes(handle->mm_handle ,NULL, MMCAM_CAMERA_DEVICE_COUNT, &camera_device_count, NULL);
	if (ret != MM_ERROR_NONE) {
		free(handle);
		handle = NULL;
...
int recorder_get_video_encoder(recorder_h recorder, recorder_video_codec_e *codec)
{
	...
	if (codec == NULL) {
		LOGE("NULL pointer codec = [%p]", codec);
		return RECORDER_ERROR_INVALID_PARAMETER;
	}
...
int recorder_attr_set_recording_motion_rate(recorder_h recorder , double rate)
{
	...
	ret = mm_camcorder_set_attributes(handle->mm_handle ,NULL, "camera-recording-motion-rate" , rate, NULL);
	return  __convert_recorder_error_code(__func__, ret);
}
...
int recorder_attr_get_recording_motion_rate(recorder_h recorder , double *rate)
{
	...
	if (rate == NULL) {
		LOGE("The value[rate] of 2nd parameter is NULL");
		return RECORDER_ERROR_INVALID_PARAMETER;
	}

	ret = mm_camcorder_get_attributes(handle->mm_handle ,NULL, "camera-recording-motion-rate" , rate , NULL);
	return  __convert_recorder_error_code(__func__, ret);
}
...
int recorder_attr_get_audio_channel(recorder_h recorder, int *channel_count){
	...
	ret = mm_camcorder_get_attributes(handle->mm_handle ,NULL, MMCAM_AUDIO_CHANNEL, channel_count, NULL);
...
int recorder_attr_set_orientation_tag(recorder_h recorder,  recorder_rotation_e orientation)
{
	...
	if (orientation > RECORDER_ROTATION_270) {
		LOGE("The value of parameter is exceeded");
		return RECORDER_ERROR_INVALID_PARAMETER;
	}
...
int  recorder_attr_get_orientation_tag(recorder_h recorder, recorder_rotation_e *orientation)
{
	...
	if (orientation == NULL) {
		LOGE("The value[orientation] of 2nd parameter is NULL");
		return RECORDER_ERROR_INVALID_PARAMETER;
	}
	ret = mm_camcorder_get_attributes(handle->mm_handle ,NULL, MMCAM_TAG_VIDEO_ORIENTATION  , orientation, NULL);
	return __convert_recorder_error_code(__func__, ret);
}
